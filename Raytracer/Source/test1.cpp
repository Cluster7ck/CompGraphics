...
void Test1::vector()
{
    ...
    
    {
        ...
        for( int i=0; i<20000; i++)
        {
            ...
            {
                TEST( "Vector triangle Intersection failed (missed intersection)! First TEST", h1 || h2 );
                
                float s = h1 ? s1 : s2;
                
                Vector intersection = o + Dir * s;
             
                TEST( "Vector triangle Intersection failed (wrong s coordinate! Second TEST",  intersection.dot(n ) <= EPSILON);
                
                h1 = o.triangleIntersection(-Dir, a, b, c, s1);
                h2 = o.triangleIntersection(-Dir, c, b, d, s2);
                TEST( "Vector triangle Intersection failed (missed intersection)! Third TEST", !h1 && !h2 );
                
            }
            else
            {
                TEST( "Vector triangle Intersection failed (missed intersection)! In else", !h1 && !h2 );
            }
